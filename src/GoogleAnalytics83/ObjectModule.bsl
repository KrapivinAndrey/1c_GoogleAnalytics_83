...
Функция ИнициализироватьПараметрыКлиентСервер(ОсновнаяФорма = Неопределено) Экспорт
	...
	АдресаВХ.Вставить("tid", ПоместитьВоВременноеХранилище("UA-101999436-2", УникальныйИдентификатор)); //ключевой параметр - трекер ид проекта в аналитике
	АдресаВХ.Вставить("cid", ПоместитьВоВременноеХранилище(ПользователиИнформационнойБазы.ТекущийПользователь().УникальныйИдентификатор, УникальныйИдентификатор)); //идентификация пользователя
...
Процедура КэшироватьОбработкуОбъект() Экспорт
	
	Если ПараметрыКлиентСерверGA.КэшироватьОбработкуОбъект Тогда
		ОбработкаОбъектВКэш(ЭтотОбъект, ПараметрыКлиентСерверGA.АдресаВХ.ОбработкаОбъект);	
	КонецЕсли;
