...
&НаКлиенте
Функция ОтправитьСтекСобытий() Экспорт
	ДатаОтправки = ТекущаяУниверсальнаяДатаВМиллисекундах();
	
	Сч					= 0;
	СтрокаПараметров 	= "";
	ЕстьОшибка			= Ложь;
	Для Каждого Данные из СтекСобытий() Цикл
		СтрокаПараметров = ПолучитьСтрокуПараметров(Данные.Значение) + "&" + СмещениеВОчереди(ДатаОтправки, Данные.Ключ) + Символы.ПС;
		Сч = Сч + 1;
		Если Сч = 10 Тогда
...
Функция ПоместитьВОчередь() Экспорт
	СтекСобытий().Вставить(ТекущаяУниверсальнаяДатаВМиллисекундах(), СтруктураДанных);
КонецФункции
...
Функция НовыйСтекСобытий()
	СтекСобытийКлиент = Новый Соответствие;
КонецФункции
