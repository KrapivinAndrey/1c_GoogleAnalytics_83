...
Функция ПолучитьСтрокуПараметров(СтруктураПараметров)
	...
	СтрокаПараметров = ОбязательныеПараметры();
	...
	СтрокаПараметров = СтрокаПараметров + "&z=" + Строка(ГСЧ.СлучайноеЧисло(1,999));
...
Функция ОбязательныеПараметры()
	ОбязательныеПараметры	= "v=1" + 
								"&tid=" + Объект.ПараметрыКлиентСерверGA.tid + 
								"&cid=" + Объект.ПараметрыКлиентСерверGA.cid;
...
Функция ПросмотрСтраницы(ИмяХоста = "", ПутьКДокументу = "", ЗаголовокДокумента = "") Экспорт
	...
	СтруктураДанных.Вставить("t",	"pageview");
	...
	Если ПустаяСтрока(ИмяХоста) Тогда
		СтруктураДанных.Удалить("dh");
	КонецЕсли;
...
&НаКлиенте
Функция ЗамерПроизводительности(КатегорияЗамера, ОписаниеЗамера, ЗначениеЗамера, ЯрлыкЗамера = "") Экспорт
	
	СтруктураДанных.Вставить("utc", КатегорияЗамера);
	СтруктураДанных.Вставить("utv", ОписаниеЗамера);
	СтруктураДанных.Вставить("utt", ЗначениеЗамера);
	СтруктураДанных.Вставить("utl", ЯрлыкЗамера);
	
	Если ПустаяСтрока(ЯрлыкЗамера) Тогда
		СтруктураДанных.Удалить("utl");
	КонецЕсли;
...
&НаКлиенте
Функция Ошибка(ОписаниеОшибки, КритическаяОшибка = Ложь) Экспорт
	СтруктураДанных.Вставить("exd", ОписаниеОшибки);
	СтруктураДанных.Вставить("exd", Формат(КритическаяОшибка, "БЛ=0; БИ=1"));
...
&НаКлиенте
Функция НовыйСтекСобытий() Экспорт
